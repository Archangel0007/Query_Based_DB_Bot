...
13.  The Create statments That will be generated by the python code should be in order such that any table that is referenced by a foreign key should be created before the table that references it. This is CRITICAL to avoid foreign key constraint errors during table creation.
14.  No tables should be missed while generating the create statements.
